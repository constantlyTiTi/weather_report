...
const WeatherTemperatureBox = ({ weatherInfo }) => {

    const { name, weather, main, wind, sys } = weatherInfo
    const { temp, feels_like, humidity } = main
    const { speed } = wind
    const { sunrise, sunset } = sys
    const dateTime = new Date().toLocaleString()
    const sunriseDateTime = new Date(sunrise * 1000).toLocaleString().split(" ")[1]
    const sunsetDateTime = new Date(sunset * 1000).toLocaleString().split(" ")[1]
    ...
    return (
        <div className="max-w-lg p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <h5 className="my-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{name}</h5>
            ...
            <div className="flex my-4">
                {
                    !!weather && weather.length > 0 ? weather.map(item => (
                        <div className="border border-gray-200">
                            <h5 className='text-center'>{item.main}</h5>
                            <img className='' src={`${GET_WEATHER_ICON}${item.icon}@2x.png`} />
                        </div>
                    )) : null
                }
